...
def soft_pseudo_gener(model_S_restore,model_T_restore,save_path,thresh_path,model_name,data_path_ori,data_path_tran,dataloader='cityscapes'):
    num_classes = 19
    S_thresh=np.load(save_path+'/perf_s.npy')
    T_thresh=np.load(save_path+'/perf_t.npy')
    print("restore thresh from %s"%(thresh_path))
    print(np.nanmean(S_thresh))
    print(np.nanmean(T_thresh))
    ...
    with torch.no_grad():
        for index, batch in enumerate(targetloader):
            ...
            output=output*w1+outputB*(1-w1)
            output = np.asarray(output, dtype=np.float16)
            name = name[0].split('/')[-1]
            # name=os.path.splitext(name.split('/')[-1])[0]+".npy"
            # np.save('%s/%s' % (save_path, name),output)
        ...
        print(thres)
        np.save(save_path+'/conf.npy',thres)
        print("saved in "+save_path+"/conf.npy")
