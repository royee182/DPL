...
class FCDiscriminator(nn.Module):
    ...
    def adjust_learning_rate(self, args, optimizer, i):
        if args.model == 'DeepLab':
            ...
            optimizer.param_groups[0]['lr'] = args.learning_rate_D * (0.1**(int(i/args.decay_step)))
            if len(optimizer.param_groups) > 1:
                optimizer.param_groups[1]['lr'] = args.learning_rate_D * (0.1**(int(i/args.decay_step))) * 2
...
def CreateDiscriminator(lr=2e-4):
    discriminator = FCDiscriminator(num_classes=19)
    optimizer = optim.Adam(discriminator.parameters(), lr=lr, betas=(0.9, 0.99))
    optimizer.zero_grad()
