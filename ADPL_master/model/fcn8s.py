...
class FCN8s(nn.Module):
    ...
    def adjust_learning_rate(self, args, optimizer, i):
        optimizer.param_groups[0]['lr'] = args.lr * (0.1**(int(i/args.decay_step)))
        if len(optimizer.param_groups) > 1:
            optimizer.param_groups[1]['lr'] = args.lr * (0.1**(int(i/args.decay_step))) * 2
    ...
    def optim_parameters(self, args):
        return [
                {'params': self.get_parameters(bias=False)},
                {'params': self.get_parameters(bias=True),
                 'lr': args.learning_rate * 2}
            ]
def VGG16_FCN8s(num_classes=21):
