...
def define_sem_D(init_type='normal', init_gain=0.02, gpu_ids=[]):
    discriminator = FCDiscriminator(num_classes=19)
    init_net(discriminator, init_type, init_gain, gpu_ids)
...
def semantic(init_weights=None, gpu_ids=0,mode='eval',model_choice='VGG'):
...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):

    if netF == 'sample':
        net = PatchSampleF(use_mlp=False, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids)
...
class PatchSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        ...
        if self.use_mlp and not self.mlp_init:
            self.create_mlp()
    def create_mlp(self):
        mlp = nn.Sequential(*[nn.Linear(19, self.nc), nn.ReLU(), nn.Linear(self.nc, self.nc)])
        mlp.cuda()
        self.mlp=mlp
