...
def compute_mIoU(gt_dir, pred_dir, devkit_dir='', log_dir='',print_iou=True):
    ...
    if print_iou==True:
        for ind_class in range(num_classes):
            with open(log_dir + "/mIoU.txt", 'a') as f:
...
def val(args=None):
    ...
    if args.set=='train':
        args.set='val'
        args.save=os.path.join(args.snapshot_dir,'results')
        args.data_list_target='./dataset/cityscapes_list/val.txt'
        args.data_label_folder_target=None
        args.gt_dir='../data/Cityscapes/gtFine/val'
        args.devkit_dir='./dataset/cityscapes_list'
        args.log_dir=args.snapshot_dir
        args.print_iou=False
    ...
    model.eval()
    model.cuda()
    targetloader = CreateTrgDataLoader(args,translated=(args.domain=='S'))
    ...
    for index, batch in enumerate(targetloader):
        ...
        # output_col.save('%s/%s_color.png' % (save, name.split('.')[0]))

    return compute_mIoU(args.gt_dir, args.save, args.devkit_dir, args.log_dir,args.print_iou)
